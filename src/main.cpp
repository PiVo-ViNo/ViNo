...
bool path_integrity(char *path)
{
	...
	if (fs::exists(_src_path) && _src_path.extension() == ".vnsf")
		return true;
...
int main(int argc, char **argv)
{
	...
	if (argc > 1)
	{
		...
		{
			std::cout << argv[i] << '\n';
			if (argc - i >= 2 && !std::strcmp(argv[i], _f_path) && path_integrity(argv[i + 1]))
			{
				_flags.set(0);
	...
	{ // needs a check if there is the running instance of program,
	  //  if there are many, give error "Many instances : cannot attach to random"
