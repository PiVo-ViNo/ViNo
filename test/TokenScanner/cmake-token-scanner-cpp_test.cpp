...
int main()
{
	...
	try
	{
		...
		const std::vector<vst> tokens_test = {vst::NEW_LINE,

											  vst::BG,		  vst::VAR,		  vst::SIGN_EQ,	  vst::TEXT_LINE, vst::NEW_LINE,

											  vst::FG,		  vst::VAR,		  vst::SIGN_EQ,	  vst::TEXT_LINE, vst::NEW_LINE,

											  vst::PERSONA,	  vst::VAR,		  vst::BRACE_OP,  vst::FG,		  vst::SIGN_EQ,	 vst::TEXT_LINE, vst::BRACE_CL, vst::NEW_LINE,
	...
	try
	{
		...
		typedef vino::ScriptToken vst;
		std::vector<vst> tokens_test = {vst::BG,  vst::VAR, vst::SIGN_EQ, vst::TEXT_LINE, vst::NEW_LINE,

										vst::EXIT};
		vino::TokenScanner tokenizer(std::move(_istr_test));
		std::vector<vino::ScriptToken> tokens_get = tokenizer.get_all_tokens(true);
		assert(tokens_get == tokens_test);
	}
	catch (std::exception &excp)
	{
